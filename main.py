...
@app.post("/api/incidents", response_model=schemas.IncidentOut, status_code=201)
def create_incident_api(
    incident: schemas.IncidentCreate,
    conn=Depends(get_db),
):
    incident_id = crud.create_incident(conn, incident)
    created = crud.get_incident(conn, incident_id)

    if not created:
        raise HTTPException(status_code=500, detail="Incident was created but could not be retrieved")
...
def update_incident_api(
    ...
    updated = crud.get_incident(conn, incident_id)

    if not updated:
        raise HTTPException(status_code=500, detail="Incident updated but could not be retrieved")
...
def add_incident_form(request: Request):
    return templates.TemplateResponse("add_incident.html", {"request": request})
